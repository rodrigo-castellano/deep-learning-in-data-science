...
def process_save_raw_data():

    data_path = os.path.join('./Dataset/', 'goblet_book.txt')
    my_file = open(data_path, "r")
    dataset = my_file.read()
    unique_chars = set(dataset)

    lista = list(range(len(unique_chars)))
    ind_to_char = dict( zip(unique_chars,lista) )
    char_to_ind = {v: k for k, v in ind_to_char.items()}

    one_hot = char2one_hot(dataset,ind_to_char)
    np.save('./Dataset/one_hot.npy',one_hot)

    a_file = open("./Dataset/unique_chars.pkl", "wb")
    pickle.dump(unique_chars, a_file)
    a_file.close()

    a_file = open("./Dataset/ind_to_char.pkl", "wb")
    pickle.dump(ind_to_char, a_file)
    a_file.close()

    a_file = open("./Dataset/char_to_ind.pkl", "wb")
    pickle.dump(char_to_ind, a_file)
    a_file.close()
...
def load_data():
    ...
    one_hot = np.load('./Dataset/one_hot.npy')

    a_file = open("./Dataset/ind_to_char.pkl", "rb")
    ind_to_char = pickle.load(a_file)

    a_file = open("./Dataset/char_to_ind.pkl", "rb")
    char_to_ind = pickle.load(a_file)

    return one_hot,dataset,ind_to_char,char_to_ind

def char2one_hot(chars,ind_to_char):
    ''' Parameters: 
...
def one_hot2char(one_hot,char_to_ind):
...
def save_state(history):
    ...
    dir,_,loss_train,_,etas = history
 
    plot_data([etas],title='Eta',xlabel='t',ylabel='value',dir=dir+'lr.png')
    plot_data([loss_train],title='Loss',xlabel='t',ylabel='Loss',dir=dir+'loss.png')
...
class optimizer:
    def __init__(self,opt,beta1=.9,beta2=.999,Adam_eps=1e-8,Adagrad_eps=1e-8):
        self.opt = opt
        if self.opt == 'Adam':
    ...
    def update_grads(self,t,eta,grads,params):
        if self.opt == 'Adam':
            ...
            params = self.update_Adagrad(eta,grads,params)
        return params
    ...
    def update_Adagrad(self,eta,grads,params):
        if self.append_gradients:
            self.append_gradients = False
            self.m = []
            for j in range(len(params)):
                self.m.append([])
                for k in range(len(params[j])):
                    self.m[j].append( grads[j][k]**2 )
                    params[j][k] = params[j][k] - eta*grads[j][k]/(np.sqrt(self.m[j][k]+self.Adagrad_eps))
        else:
            for j in range(len(params)):
                for k in range(len(params[j])):
                    self.m[j][k] += grads[j][k]**2
                    params[j][k] = params[j][k] - eta*grads[j][k]/(np.sqrt(self.m[j][k]+self.Adagrad_eps))
        return params
    ...
    def param_info(self):
        if self.opt == 'Adam':
            ...
            info = {'optimizer':self.opt,'Adagrad_eps':self.Adagrad_eps}
        return info
...
class lr_scheduler:
    ...
    def param_info(self):
        if self.lr_sch == 'decay':
            ...
            info = {'scheduler':self.lr_sch,'eta':self.eta}
        return info
