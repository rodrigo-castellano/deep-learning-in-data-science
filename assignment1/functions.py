...
class model:
    def __init__(self,train_set, val_set, test_set):
        ...
        self.X_val = val_set['data']
        self.X_test = test_set['data']

        self.Y_train = train_set['one_hot']
        self.Y_val = val_set['one_hot']
        self.Y_test = test_set['one_hot']
    ...
    def zero_mean(self):
        ...
        mean = np.mean(self.X_train, axis=1)
        std = np.std(self.X_train, axis=1)
    ...
    def train_model(self,n_batch=100,n_epochs=40,landa=0,eta=.1, lr_epochs = 30,lr_decay = False):
        ...
        return W_star, b_star,loss_train,loss_val,cost_train,cost_val, test_acc,val_acc

    def GridSearch(self, param_grid,X, Y,X_val,Y_val,X_test,Y_test, W, b,lr_epochs=5, lr_decay=False):
        from sklearn.model_selection  import ParameterGrid
        grid = ParameterGrid(param_grid)
        root = time.strftime("%Y-%m-%d_%H-%M-%S_grid_search")
        dir ='./results/'+root+'/' 
        ...
        for i,params in enumerate(grid):
            ...
            W_star, b_star,loss_train,loss_val,cost_train,cost_val, test_acc,train_acc,val_acc = self.MiniBatchGD(X, Y,X_val,Y_val,X_test,Y_test,\
                             GDparams, W, b, landa,lr_epochs, lr_decay)
            acc = self.ComputeAccuracy(X_test, Y_test,W_star, b_star)
            all_acc.append(acc)
            params.update({'accuracy':acc})
            self.save_state(root+'/'+timestr,W_star,b_star,cost_train,cost_val,loss_train,loss_val,GDparams,landa,test_acc,train_acc,val_acc)
